function writeUTFBytes(dataview: DataView, offset: number, str: string) {
  for (let i = 0; i < str.length; i++) {
    dataview.setUint8(offset + i, str.charCodeAt(i));
  }
}

function mergeBuffers(buffer: Float32Array[], length: number): Float64Array {
  const result = new Float64Array(length);
  let offset = 0;
  // tslint:disable-next-line:prefer-for-of
  for (let i = 0; i < buffer.length; i++) { // Use for-each instead!
